...
export default function PhotosUploader({ addedPhotos, setAddedPhotos }) {
  ...
  const removePhoto = (e, link) => {
    e.preventDefault();
    const newPhotoArray = addedPhotos.filter((value) => value !== link);
    setAddedPhotos(newPhotoArray);
  };

  const selectAsMainPhoto = (e, link) => {
    e.preventDefault();
    const newPhotoArray = addedPhotos.filter((value) => value !== link);
    setAddedPhotos([link, ...newPhotoArray]);
  };
      ...
      <div className="mt-4 grid gap-2 grid-cols-3 md:grid-cols-4 ">
        {addedPhotos.length > 0 &&
          addedPhotos.map((link) => (
            <div className="flex h-48" key={link}>
              <img
              ...
              <button
                ...
                className="absolute p-1 rounded-full m-3 text-white bg-red-500 cursor-pointer"
              >
                <svg
              ...
              <button
                ...
                className="absolute p-1 mt-3 ml-12 rounded-full text-white bg-blue-500 cursor-pointer"
              >
                {link === addedPhotos[0] ? (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
